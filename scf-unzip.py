...
Returns:
- On success: {"status":"ok","bucket":<event bucket short name>,"source_key":<zip key>,"output_prefix":<dest prefix>}
...
def _extract_and_upload(zip_path: str, bucket: str, dest_prefix: str):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            for member in zf.infolist():
                ...
                if clean_name.endswith('/'):
                    ...
                    continue
                # Read entry data into memory; for very large files consider streaming to temp
    ...
    try:
        ...
        for rec in records:
            ...
            if len(parts) >= 3 and parts[0].isdigit():
                # bucket short name before '-' (e.g., example-123456 -> example)
                bucket_short = (cos_bucket or '').split('-')[0]
                if parts[1] == bucket_short:
                    key = '/'.join(parts[2:])
        ...
        _extract_and_upload(tmp_zip, bucket_to_use, dest_prefix)
